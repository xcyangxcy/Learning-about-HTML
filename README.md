...
3、绝对定位：语法：选择器{position：absolute}，若此盒子没有祖先元素或祖先元素没有定位，则以浏览器为标准进行移动；若有祖先元素，则以最近一级的有定位的元素为标准进行移动。绝对定位脱离标准流，不占有原来的位置。绝对定位居中算法：先left：50%走父盒子一半的宽度，再margin-left:负值  走定位盒子一半的宽度<br>
